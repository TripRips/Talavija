...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-dark text-white">Labot Tiesības</div>
            ...
            <form x-data="laratrustForm()" x-init="{!! $model ? '' : '$watch(\'displayName\', value => onChangeDisplayName(value))'!!}" method="POST" action="{{$model ? route("update", $model->id) : route("laratrust.{$type}s.store")}}">
                @csrf
                @if ($model)
                    @method('PUT')
                @endif
                ...
                <div class="flex justify-end">
                    <a href="{{url('/permissions')}}" class="btn btn-warning">Atcelt</a>
