...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-dark text-white">Pievienot Grupas</div>
        <div class="card-body">

            <form x-data="laratrustForm()" x-init="{!! $model ? '' : '$watch(\'displayName\', value => onChangeDisplayName(value))'!!}" method="POST" action="{{$model ? route("createroles", $model->id) : route("rolestore")}}">
                @csrf
                @if ($model)
                    @method('PUT')
                @endif
                ...
                <div class="form-group row ">
                    <label class="col-sm-5 col-form-label">Nosaukums/Kods
                        <div class="form-group">
                            <input class="form-control @error('name') border-red-500 @enderror" name="name" placeholder="this-will-be-the-code-name" value="{{$model->display_name ?? old('display_name') }}" readonly autocomplete="off">
                            @error('name')
                ...
                <div class="flex justify-end">
                    ...
                    <button class="btn btn-info" type="submit">Pievienot</button>
                </div>
            </form>
        </div>
    </div>
